...
// Set the column headers with styling
$sheet->setTitle('Job Report');
...
// Set the data rows with alternating row colors and borders
$rowIndex = 2;
foreach ($jobs as $job) {
    $columnIndex = 1;
    foreach ($columns as $column) {
        ...
        $sheet->getStyleByColumnAndRow($columnIndex, $rowIndex)->applyFromArray([
            'borders' => [
                'allBorders' => [
                    'borderStyle' => Border::BORDER_THIN,
                    'color' => ['rgb' => '000000'],
            ...
            'fill' => [
                'fillType' => Fill::FILL_SOLID,
                'startColor' => ['rgb' => ($rowIndex % 2 == 0) ? 'F0F0F0' : 'FFFFFF'],
            ],
        ]);
...
// Set the file name with search criteria
$fileName = 'job_report_';
if (!empty($client)) $fileName .= 'client_' . $client . '_';
if (!empty($jobNumber)) $fileName .= 'job_' . $jobNumber . '_';
if (!empty($year)) $fileName .= 'year_' . $year . '_';
if (!empty($typeOfWork)) $fileName .= 'type_' . $typeOfWork . '_';
